...
export default function AppLayout({ children, title = 'نظام إدارة المبيعات' }) {
    ...
    const getCurrentPageInfo = () => {
        const currentItem = navigation.find(item => url.startsWith(item.href) && item.href !== '/') ||
                           navigation.find(item => item.href === '/' && url === '/') ||
                           navigation[0];
        return currentItem;
    };
                ...
                <div className="px-3 sm:px-4 md:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-12 sm:h-14">
                        ...
                        <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4 space-x-reverse">
                            ...
                            {isMobile && (
                                <button
                                    ...
                                    className="p-1.5 sm:p-2 rounded-lg text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-20"
                                >
                            ...
                            <div className="flex items-center space-x-2 sm:space-x-3 space-x-reverse">
                                <div className="w-6 h-6 sm:w-8 sm:h-8 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg flex items-center justify-center shadow-sm">
                                ...
                                <div className="hidden xs:block sm:block">
                                    <h1 className="text-xs sm:text-sm font-semibold text-gray-900">
                                        DubaiExchange - {user?.name || 'نظام المبيعات'}
                                    </h1>
                        ...
                        <div className="hidden md:flex flex-1 max-w-md mx-8">
                            <div className="relative w-full">
                                <input
                                    type="text"
                                    placeholder="البحث..."
                                    className="w-full pl-10 pr-4 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-20 focus:outline-none transition-all duration-200"
                                />
                        ...
                        <div className="flex items-center space-x-1 sm:space-x-2 md:space-x-3 space-x-reverse">
                            ...
                            {/* Notifications */}
                            <button className="relative p-1.5 sm:p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-20">
                                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div className="flex items-center space-x-1 sm:space-x-2 space-x-reverse">
                                ...
                                <div className="w-6 h-6 sm:w-8 sm:h-8 bg-gradient-to-br from-indigo-400 to-purple-500 rounded-full flex items-center justify-center text-white text-xs sm:text-sm font-semibold">
                ...
                style={{ height: isMobile ? '100vh' : 'calc(100vh - 56px)' }}>
                    <div className="flex flex-col h-full bg-white">
                        ...
                        <div className="p-3 sm:p-4 border-b border-gray-100 flex-shrink-0">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-2 sm:space-x-3 space-x-reverse">
                                    ...
                                    <div>
                                        <h2 className="text-sm sm:text-sm font-semibold text-gray-900">نظام المبيعات</h2>
                                ...
                                {navigation.map((item) => (
                                    <Link
                                        ...
                                        className={`w-full flex items-center px-3 sm:px-4 py-3.5 sm:py-4 md:py-2.5 text-right rounded-xl transition-all duration-200 group ${
                                            (url.startsWith(item.href) && item.href !== '/') || (item.href === '/' && url === '/')
                                                ? 'bg-indigo-50 text-indigo-700 shadow-sm border-r-2 border-indigo-500'
                                                : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 active:bg-gray-100'
                                        }`}
                                    >
                                        <svg
                                            className={`ml-2.5 sm:ml-3 w-5 h-5 sm:w-6 sm:h-6 md:w-5 md:h-5 ${
                                                (url.startsWith(item.href) && item.href !== '/') || (item.href === '/' && url === '/') ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-600'
                                        ...
                                        <div className="flex-1 text-right">
                                            <div className={`text-sm sm:text-base md:text-sm font-medium ${((url.startsWith(item.href) && item.href !== '/') || (item.href === '/' && url === '/')) ? 'text-indigo-900' : ''}`}>
                                                {item.name}
                                            </div>
                                            <div className={`text-xs sm:text-sm md:text-xs mt-0.5 sm:mt-1 md:mt-0.5 ${
                                                ((url.startsWith(item.href) && item.href !== '/') || (item.href === '/' && url === '/')) ? 'text-indigo-600' : 'text-gray-500'
                                            }`}>
                                                {item.description}
                                            </div>
                                        </div>
                                        {((url.startsWith(item.href) && item.href !== '/') || (item.href === '/' && url === '/')) && (
                                            <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-2 md:h-2 bg-indigo-500 rounded-full"></div>
                                        )}
                                    </Link>
                                ))}
                            </nav>
                        ...
                        <div className="p-3 sm:p-4 border-t border-gray-100 flex-shrink-0 bg-white">
                            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-2.5 sm:p-3 border border-indigo-100">
                                <div className="flex items-center space-x-2 space-x-reverse">
                                    <div className="w-5 h-5 sm:w-6 sm:h-6 bg-indigo-500 rounded-lg flex items-center justify-center flex-shrink-0">
                                        <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                        </svg>
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs font-medium text-indigo-900 truncate">النظام الرئيسي</p>
                                        <p className="text-xs text-indigo-600 truncate">جميع الخدمات متاحة</p>
                                    </div>
                                </div>
                            </div>
                        </div>
