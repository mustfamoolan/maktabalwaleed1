...
import { FaArrowRight, FaBoxOpen, FaUser, FaPhone, FaDollarSign, FaListUl, FaCheckCircle } from 'react-icons/fa';
...
export default function ShowInvoice({ invoice }) {
    ...
    // حالة تجهيز كل منتج
    const [prepared, setPrepared] = useState(() => data.items.map(() => false));
    const [saving, setSaving] = useState(false);
    const printRef = useRef();

    // عند تغيير حالة تجهيز منتج
    const togglePrepared = (idx) => {
        setPrepared((prev) => prev.map((v, i) => (i === idx ? !v : v)));
    };

    // حفظ تجهيز المنتجات (يمكن ربطه بالباك)
    const handleSave = () => {
        setSaving(true);
        router.patch(`/preparer/invoices/${invoice.id}/complete`, {
            ...
            onSuccess: () => {
                setSaving(false);
                window.location.href = '/preparer/invoices/preparing';
            },
            onError: () => {
                setSaving(false);
    ...
    const handlePrint = () => {
        ...
        win.document.write('<html><head><title>طباعة الفاتورة</title>');
        win.document.write('<style>body{direction:rtl;font-family:tahoma,Arial,sans-serif;} table{border-collapse:collapse;width:100%;} th,td{border:1px solid #ccc;padding:4px;text-align:center;} .text-center{text-align:center;}</style>');
        ...
        <PreparerLayout title={`تفاصيل الفاتورة ${data.sale_number}`}>
            ...
            <div className="max-w-md mx-auto p-2 sm:p-4 space-y-4">
                ...
                <div className="flex items-center justify-between mb-2">
                    <Link href="/preparer/invoices/preparing" className="text-blue-600 flex items-center text-sm">
                        <FaArrowRight className="ml-1" /> رجوع
                    </Link>
                    <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full">{data.status === 'preparing' ? 'قيد التجهيز' : ''}</span>
                </div>
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 space-y-2">
                    ...
                    <div className="flex justify-end mb-2">
                        <button
                            onClick={handlePrint}
                            className="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm shadow"
                        >
                            طباعة الفاتورة
                        </button>
                    </div>
                    <div className="flex items-center space-x-2 space-x-reverse mb-2">
                        <FaListUl className="w-5 h-5 text-blue-500" />
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                        ...
                        {data.items.map((item, idx) => (
                            <li key={item.id} className="py-2 flex items-center justify-between">
                                <div className="flex items-center">
                                    <input
                                        type="checkbox"
                                        checked={prepared[idx]}
                                        onChange={() => togglePrepared(idx)}
                                        className="form-checkbox h-5 w-5 text-green-600 border-gray-300 rounded ml-2"
                                    />
                                    {/* صورة المنتج */}
                                    <img
                                        src={
                    ...
                    <button
                        onClick={handleSave}
                        disabled={saving}
                        className="w-full mt-4 flex items-center justify-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {saving ? (
                            <span className="animate-spin mr-2"><FaCheckCircle /></span>
                        ) : (
