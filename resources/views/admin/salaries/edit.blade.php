...
@section('title', 'تعديل الراتب - نظام إدارة المبيعات')

@section('page-title', 'تعديل الراتب')
...
@section('content')
    <div class="main-split-container">
        ...
        <div class="salary-main-section">
            <form id="salaryForm" action="{{ route('admin.salaries.update', $salary->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="salary-form-section">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <select name="representative_id" id="representative_id" class="win-input" required>
                                ...
                                @foreach($representatives as $representative)
                                    <option value="{{ $representative->id }}" {{ ($salary->representative_id == $representative->id) ? 'selected' : '' }}>
                                        {{ $representative->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="basic_salary" id="basic_salary" class="win-input" value="{{ $salary->basic_salary }}" min="0" step="1000" required>
                            @error('basic_salary')
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="date" name="effective_from" id="effective_from" class="win-input" value="{{ $salary->effective_from->format('Y-m-d') }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="effective_to" id="effective_to" class="win-input" value="{{ $salary->effective_to ? $salary->effective_to->format('Y-m-d') : '' }}">
                            @error('effective_to')
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            <div class="checkbox-group">
                                <label class="checkbox-label">
                                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ $salary->is_active ? 'checked' : '' }}>
            ...
            <div class="sidebar-content">
                <div class="preview-section">
                    ...
                    <div class="preview-item">
                        <label>المندوب:</label>
                        <span id="preview-representative">{{ $salary->representative->name }}</span>
                    </div>

                    <div class="preview-item">
                        <label>الراتب الأساسي:</label>
                        <span id="preview-basic-salary">{{ number_format($salary->basic_salary, 0) }} د.ع</span>
                    </div>

                    <div class="preview-item">
                        <label>تاريخ البداية:</label>
                        <span id="preview-effective-from">{{ $salary->effective_from->format('Y-m-d') }}</span>
                    </div>

                    <div class="preview-item">
                        <label>تاريخ النهاية:</label>
                        <span id="preview-effective-to">{{ $salary->effective_to ? $salary->effective_to->format('Y-m-d') : 'مفتوح' }}</span>
                    </div>

                    <div class="preview-item">
                        <label>الحالة:</label>
                        <span id="preview-status">{{ $salary->is_active ? 'نشط' : 'غير نشط' }}</span>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // تفعيل التحقق للحقول الممتلئة مسبقاً
    validateField('representative', document.getElementById('representative_id').value !== '');
    validateField('salary', parseFloat(document.getElementById('basic_salary').value) > 0);
    validateField('date', document.getElementById('effective_from').value !== '');
});
</script>
@endsection
